...
export class TaskViewPanel {
  ...
  private _buildHtml(): string {
    ...
    const rows = all.length === 0
      ? `<tr><td colspan="5" class="empty">${t('tv_empty')}</td></tr>`
...
<div class="header">
  <h2>${t('tv_title')}</h2>
  <button class="btn" id="clearBtn">${t('tv_clearCompleted')}</button>
</div>
<table>
<thead><tr>
  <th></th><th>${t('wv_name')}</th><th>${t('tv_progress')}</th><th>${t('tv_status')}</th><th>${t('tv_time')}</th>
</tr></thead>
<tbody>${rows}</tbody>
</table>
<script>
...
document.getElementById('clearBtn').addEventListener('click', () => {
  ...
  private _statusLabel(task: Task): string {
    switch (task.status) {
      case 'in_progress': return t('tv_inProgress');
      case 'completed': return t('tv_completed');
      case 'failed': return t('tv_failed');
      case 'interrupted': return t('tv_interrupted');
